...
public class FileLoader : MonoBehaviour
{
    ...
    public IEnumerator DownloadImageCoroutine(string url, string path, bool shouldFileBeSaved, Action<Texture2D> OnTextureLoaded)
    {
        ...
        if(File.Exists(path))
        {
            ...
            loadedTexture.LoadImage(decryptedImageBytes);
    ...
    public IEnumerator DownloadTextCoroutine(string url, string path, bool shouldFileBeSaved, Action<string> OnTextLoaded)
    {
        string text;
        if(File.Exists(path))
        {
            ...
            text = Encoding.UTF8.GetString(decryptedTextBytes); 
            OnTextLoaded?.Invoke(text);
            //text = File.ReadAllText(path); saves the file without encrypting it
        }
        else
        {
            using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
